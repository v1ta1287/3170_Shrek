transcript = {
  "transcripts": [
    {
      "confidence": -977.0841064453125,
      "words": [
        {
          "word": "that",
          "start_time": 0.48,
          "duration": 0.08
        },
        {
          "word": "was",
          "start_time": 0.6,
          "duration": 0.1
        },
        {
          "word": "actually",
          "start_time": 0.78,
          "duration": 0.28
        },
        {
          "word": "example",
          "start_time": 1.12,
          "duration": 0.4
        },
        {
          "word": "of",
          "start_time": 1.58,
          "duration": 0.08
        },
        {
          "word": "how",
          "start_time": 1.76,
          "duration": 0.16
        },
        {
          "word": "not",
          "start_time": 2.0,
          "duration": 0.26
        },
        {
          "word": "so",
          "start_time": 2.3,
          "duration": 0.14
        },
        {
          "word": "amazing",
          "start_time": 2.52,
          "duration": 0.34
        },
        {
          "word": "he",
          "start_time": 2.98,
          "duration": 0.12
        },
        {
          "word": "was",
          "start_time": 3.16,
          "duration": 0.18
        },
        {
          "word": "but",
          "start_time": 3.42,
          "duration": 0.12
        },
        {
          "word": "you",
          "start_time": 3.56,
          "duration": 0.12
        },
        {
          "word": "really",
          "start_time": 3.72,
          "duration": 0.18
        },
        {
          "word": "going",
          "start_time": 3.94,
          "duration": 0.22
        },
        {
          "word": "through",
          "start_time": 4.2,
          "duration": 0.24
        },
        {
          "word": "something",
          "start_time": 4.54,
          "duration": 0.32
        },
        {
          "word": "here",
          "start_time": 4.94,
          "duration": 1.88
        },
        {
          "word": "now",
          "start_time": 6.92,
          "duration": 1.1
        },
        {
          "word": "i",
          "start_time": 8.28,
          "duration": 0.18
        },
        {
          "word": "find",
          "start_time": 8.54,
          "duration": 0.54
        },
        {
          "word": "all",
          "start_time": 9.22,
          "duration": 0.1
        },
        {
          "word": "those",
          "start_time": 9.38,
          "duration": 0.2
        },
        {
          "word": "years",
          "start_time": 9.68,
          "duration": 0.26
        },
        {
          "word": "when",
          "start_time": 10.0,
          "duration": 0.1
        },
        {
          "word": "i",
          "start_time": 10.14,
          "duration": 0.06
        },
        {
          "word": "pretended",
          "start_time": 10.26,
          "duration": 0.34
        },
        {
          "word": "to",
          "start_time": 10.64,
          "duration": 0.1
        },
        {
          "word": "cry",
          "start_time": 10.8,
          "duration": 0.2
        },
        {
          "word": "i",
          "start_time": 11.04,
          "duration": 0.02
        },
        {
          "word": "used",
          "start_time": 11.14,
          "duration": 0.12
        },
        {
          "word": "to",
          "start_time": 11.28,
          "duration": 0.08
        },
        {
          "word": "use",
          "start_time": 11.44,
          "duration": 0.14
        },
        {
          "word": "dad's",
          "start_time": 11.64,
          "duration": 0.28
        },
        {
          "word": "death",
          "start_time": 12.0,
          "duration": 0.2
        },
        {
          "word": "to",
          "start_time": 12.24,
          "duration": 0.08
        },
        {
          "word": "get",
          "start_time": 12.36,
          "duration": 0.12
        },
        {
          "word": "me",
          "start_time": 12.52,
          "duration": 0.12
        },
        {
          "word": "going",
          "start_time": 12.72,
          "duration": 0.34
        },
        {
          "word": "i",
          "start_time": 13.12,
          "duration": 0.06
        },
        {
          "word": "try",
          "start_time": 13.22,
          "duration": 0.1
        },
        {
          "word": "with",
          "start_time": 13.44,
          "duration": 0.12
        },
        {
          "word": "mom",
          "start_time": 13.62,
          "duration": 0.3
        },
        {
          "word": "but",
          "start_time": 13.96,
          "duration": 0.16
        },
        {
          "word": "i",
          "start_time": 14.18,
          "duration": 0.08
        },
        {
          "word": "just",
          "start_time": 14.28,
          "duration": 0.12
        },
        {
          "word": "end",
          "start_time": 14.52,
          "duration": 0.08
        },
        {
          "word": "up",
          "start_time": 14.68,
          "duration": 0.2
        },
        {
          "word": "smiling",
          "start_time": 14.96,
          "duration": 0.38
        },
        {
          "word": "and",
          "start_time": 15.42,
          "duration": 0.06
        },
        {
          "word": "ruining",
          "start_time": 15.58,
          "duration": 0.32
        },
        {
          "word": "it",
          "start_time": 15.96,
          "duration": 0.16
        },
        {
          "word": "but",
          "start_time": 16.18,
          "duration": 1.02
        },
        {
          "word": "it",
          "start_time": 17.38,
          "duration": 0.08
        },
        {
          "word": "feels",
          "start_time": 17.52,
          "duration": 0.24
        },
        {
          "word": "like",
          "start_time": 17.84,
          "duration": 0.26
        },
        {
          "word": "real",
          "start_time": 18.2,
          "duration": 0.3
        },
        {
          "word": "you",
          "start_time": 18.62,
          "duration": 0.12
        },
        {
          "word": "know",
          "start_time": 18.78,
          "duration": 0.28
        },
        {
          "word": "and",
          "start_time": 19.22,
          "duration": 0.22
        },
        {
          "word": "you",
          "start_time": 19.48,
          "duration": 0.12
        },
        {
          "word": "haven't",
          "start_time": 19.64,
          "duration": 0.26
        },
        {
          "word": "really",
          "start_time": 19.96,
          "duration": 0.24
        },
        {
          "word": "allowed",
          "start_time": 20.3,
          "duration": 0.24
        },
        {
          "word": "herself",
          "start_time": 20.58,
          "duration": 0.38
        },
        {
          "word": "to",
          "start_time": 21.02,
          "duration": 0.14
        },
        {
          "word": "grieve",
          "start_time": 21.22,
          "duration": 0.32
        },
        {
          "word": "much",
          "start_time": 21.64,
          "duration": 0.14
        },
        {
          "word": "at",
          "start_time": 21.84,
          "duration": 0.08
        },
        {
          "word": "all",
          "start_time": 22.0,
          "duration": 0.84
        },
        {
          "word": "moralists",
          "start_time": 22.9,
          "duration": 1.2
        },
        {
          "word": "much",
          "start_time": 24.14,
          "duration": 0.22
        },
        {
          "word": "more",
          "start_time": 24.42,
          "duration": 0.92
        },
        {
          "word": "what",
          "start_time": 25.4,
          "duration": 0.16
        },
        {
          "word": "more",
          "start_time": 25.62,
          "duration": 0.14
        },
        {
          "word": "complicated",
          "start_time": 25.82,
          "duration": 0.52
        },
        {
          "word": "than",
          "start_time": 26.38,
          "duration": 0.08
        },
        {
          "word": "yours",
          "start_time": 26.5,
          "duration": 0.18
        },
        {
          "word": "is",
          "start_time": 26.74,
          "duration": 0.1
        },
        {
          "word": "predicated",
          "start_time": 26.94,
          "duration": 0.38
        },
        {
          "word": "will",
          "start_time": 27.42,
          "duration": 0.14
        },
        {
          "word": "unsecured",
          "start_time": 27.6,
          "duration": 0.5
        },
        {
          "word": "and",
          "start_time": 28.18,
          "duration": 0.1
        },
        {
          "word": "lies",
          "start_time": 28.36,
          "duration": 0.2
        },
        {
          "word": "it",
          "start_time": 28.62,
          "duration": 0.06
        },
        {
          "word": "wasn't",
          "start_time": 28.7,
          "duration": 0.24
        },
        {
          "word": "a",
          "start_time": 28.96,
          "duration": 0.04
        },
        {
          "word": "whole",
          "start_time": 29.04,
          "duration": 0.14
        },
        {
          "word": "lot",
          "start_time": 29.22,
          "duration": 0.12
        },
        {
          "word": "of",
          "start_time": 29.38,
          "duration": 0.94
        },
        {
          "word": "trust",
          "start_time": 30.42,
          "duration": 0.3
        },
        {
          "word": "there",
          "start_time": 30.78,
          "duration": 0.24
        },
        {
          "word": "you",
          "start_time": 31.08,
          "duration": 0.1
        },
        {
          "word": "know",
          "start_time": 31.22,
          "duration": 0.76
        },
        {
          "word": "heroes",
          "start_time": 32.06,
          "duration": 1.44
        },
        {
          "word": "here",
          "start_time": 33.56,
          "duration": 0.22
        },
        {
          "word": "comes",
          "start_time": 33.84,
          "duration": 0.34
        },
        {
          "word": "john",
          "start_time": 34.3,
          "duration": 0.24
        },
        {
          "word": "wayne",
          "start_time": 34.66,
          "duration": 2.38
        },
        {
          "word": "i'm",
          "start_time": 37.16,
          "duration": 0.14
        },
        {
          "word": "not",
          "start_time": 37.34,
          "duration": 0.2
        },
        {
          "word": "going",
          "start_time": 37.58,
          "duration": 0.12
        },
        {
          "word": "to",
          "start_time": 37.72,
          "duration": 0.18
        },
        {
          "word": "cry",
          "start_time": 38.06,
          "duration": 0.34
        },
        {
          "word": "about",
          "start_time": 38.5,
          "duration": 0.34
        },
        {
          "word": "a",
          "start_time": 39.5,
          "duration": 0.68
        },
        {
          "word": "i'm",
          "start_time": 40.28,
          "duration": 0.12
        },
        {
          "word": "going",
          "start_time": 40.42,
          "duration": 0.12
        },
        {
          "word": "to",
          "start_time": 40.56,
          "duration": 0.14
        },
        {
          "word": "build",
          "start_time": 40.76,
          "duration": 0.22
        },
        {
          "word": "airport",
          "start_time": 41.18,
          "duration": 0.74
        },
        {
          "word": "butcherings",
          "start_time": 41.96,
          "duration": 6.42
        },
        {
          "word": "you",
          "start_time": 48.42,
          "duration": 0.1
        },
        {
          "word": "can",
          "start_time": 48.56,
          "duration": 0.1
        },
        {
          "word": "keep",
          "start_time": 48.7,
          "duration": 0.14
        },
        {
          "word": "him",
          "start_time": 48.9,
          "duration": 0.14
        },
        {
          "word": "bottled",
          "start_time": 49.06,
          "duration": 0.3
        },
        {
          "word": "up",
          "start_time": 49.44,
          "duration": 0.12
        },
        {
          "word": "but",
          "start_time": 49.6,
          "duration": 0.1
        },
        {
          "word": "they",
          "start_time": 49.74,
          "duration": 0.2
        },
        {
          "word": "will",
          "start_time": 50.02,
          "duration": 0.24
        },
        {
          "word": "come",
          "start_time": 50.32,
          "duration": 0.12
        },
        {
          "word": "out",
          "start_time": 50.52,
          "duration": 0.16
        },
        {
          "word": "michael",
          "start_time": 50.72,
          "duration": 0.3
        },
        {
          "word": "sometimes",
          "start_time": 51.08,
          "duration": 0.64
        },
        {
          "word": "in",
          "start_time": 51.78,
          "duration": 0.06
        },
        {
          "word": "the",
          "start_time": 51.88,
          "duration": 0.1
        },
        {
          "word": "most",
          "start_time": 52.02,
          "duration": 0.36
        },
        {
          "word": "conceited",
          "start_time": 52.54,
          "duration": 4.42
        }
      ]
    },
    {
      "confidence": -977.8377685546875,
      "words": [
        {
          "word": "that",
          "start_time": 0.48,
          "duration": 0.08
        },
        {
          "word": "was",
          "start_time": 0.6,
          "duration": 0.1
        },
        {
          "word": "actually",
          "start_time": 0.78,
          "duration": 0.28
        },
        {
          "word": "example",
          "start_time": 1.12,
          "duration": 0.4
        },
        {
          "word": "of",
          "start_time": 1.58,
          "duration": 0.08
        },
        {
          "word": "how",
          "start_time": 1.76,
          "duration": 0.16
        },
        {
          "word": "not",
          "start_time": 2.0,
          "duration": 0.26
        },
        {
          "word": "so",
          "start_time": 2.3,
          "duration": 0.14
        },
        {
          "word": "amazing",
          "start_time": 2.52,
          "duration": 0.34
        },
        {
          "word": "he",
          "start_time": 2.98,
          "duration": 0.12
        },
        {
          "word": "was",
          "start_time": 3.16,
          "duration": 0.18
        },
        {
          "word": "but",
          "start_time": 3.42,
          "duration": 0.12
        },
        {
          "word": "you",
          "start_time": 3.56,
          "duration": 0.12
        },
        {
          "word": "really",
          "start_time": 3.72,
          "duration": 0.18
        },
        {
          "word": "going",
          "start_time": 3.94,
          "duration": 0.22
        },
        {
          "word": "through",
          "start_time": 4.2,
          "duration": 0.24
        },
        {
          "word": "something",
          "start_time": 4.54,
          "duration": 0.32
        },
        {
          "word": "here",
          "start_time": 4.94,
          "duration": 1.88
        },
        {
          "word": "now",
          "start_time": 6.92,
          "duration": 1.1
        },
        {
          "word": "i",
          "start_time": 8.28,
          "duration": 0.18
        },
        {
          "word": "find",
          "start_time": 8.54,
          "duration": 0.54
        },
        {
          "word": "all",
          "start_time": 9.22,
          "duration": 0.1
        },
        {
          "word": "those",
          "start_time": 9.38,
          "duration": 0.2
        },
        {
          "word": "years",
          "start_time": 9.68,
          "duration": 0.26
        },
        {
          "word": "when",
          "start_time": 10.0,
          "duration": 0.1
        },
        {
          "word": "i",
          "start_time": 10.14,
          "duration": 0.06
        },
        {
          "word": "pretended",
          "start_time": 10.26,
          "duration": 0.34
        },
        {
          "word": "to",
          "start_time": 10.64,
          "duration": 0.1
        },
        {
          "word": "cry",
          "start_time": 10.8,
          "duration": 0.2
        },
        {
          "word": "i",
          "start_time": 11.04,
          "duration": 0.02
        },
        {
          "word": "used",
          "start_time": 11.14,
          "duration": 0.12
        },
        {
          "word": "to",
          "start_time": 11.28,
          "duration": 0.08
        },
        {
          "word": "use",
          "start_time": 11.44,
          "duration": 0.14
        },
        {
          "word": "dad's",
          "start_time": 11.64,
          "duration": 0.28
        },
        {
          "word": "death",
          "start_time": 12.0,
          "duration": 0.2
        },
        {
          "word": "to",
          "start_time": 12.24,
          "duration": 0.08
        },
        {
          "word": "get",
          "start_time": 12.36,
          "duration": 0.12
        },
        {
          "word": "me",
          "start_time": 12.52,
          "duration": 0.12
        },
        {
          "word": "going",
          "start_time": 12.72,
          "duration": 0.34
        },
        {
          "word": "i",
          "start_time": 13.12,
          "duration": 0.06
        },
        {
          "word": "try",
          "start_time": 13.22,
          "duration": 0.1
        },
        {
          "word": "with",
          "start_time": 13.44,
          "duration": 0.12
        },
        {
          "word": "mom",
          "start_time": 13.62,
          "duration": 0.3
        },
        {
          "word": "but",
          "start_time": 13.96,
          "duration": 0.16
        },
        {
          "word": "i",
          "start_time": 14.18,
          "duration": 0.08
        },
        {
          "word": "just",
          "start_time": 14.28,
          "duration": 0.12
        },
        {
          "word": "end",
          "start_time": 14.52,
          "duration": 0.08
        },
        {
          "word": "up",
          "start_time": 14.68,
          "duration": 0.2
        },
        {
          "word": "smiling",
          "start_time": 14.96,
          "duration": 0.38
        },
        {
          "word": "and",
          "start_time": 15.42,
          "duration": 0.06
        },
        {
          "word": "ruining",
          "start_time": 15.58,
          "duration": 0.32
        },
        {
          "word": "it",
          "start_time": 15.96,
          "duration": 0.16
        },
        {
          "word": "but",
          "start_time": 16.18,
          "duration": 1.02
        },
        {
          "word": "it",
          "start_time": 17.38,
          "duration": 0.08
        },
        {
          "word": "feels",
          "start_time": 17.52,
          "duration": 0.24
        },
        {
          "word": "like",
          "start_time": 17.84,
          "duration": 0.26
        },
        {
          "word": "real",
          "start_time": 18.2,
          "duration": 0.3
        },
        {
          "word": "you",
          "start_time": 18.62,
          "duration": 0.12
        },
        {
          "word": "know",
          "start_time": 18.78,
          "duration": 0.28
        },
        {
          "word": "and",
          "start_time": 19.22,
          "duration": 0.22
        },
        {
          "word": "you",
          "start_time": 19.48,
          "duration": 0.12
        },
        {
          "word": "haven't",
          "start_time": 19.64,
          "duration": 0.26
        },
        {
          "word": "really",
          "start_time": 19.96,
          "duration": 0.24
        },
        {
          "word": "allowed",
          "start_time": 20.3,
          "duration": 0.24
        },
        {
          "word": "herself",
          "start_time": 20.58,
          "duration": 0.38
        },
        {
          "word": "to",
          "start_time": 21.02,
          "duration": 0.14
        },
        {
          "word": "grieve",
          "start_time": 21.22,
          "duration": 0.32
        },
        {
          "word": "much",
          "start_time": 21.64,
          "duration": 0.14
        },
        {
          "word": "at",
          "start_time": 21.84,
          "duration": 0.08
        },
        {
          "word": "all",
          "start_time": 22.0,
          "duration": 0.84
        },
        {
          "word": "moralists",
          "start_time": 22.9,
          "duration": 1.2
        },
        {
          "word": "much",
          "start_time": 24.14,
          "duration": 0.22
        },
        {
          "word": "more",
          "start_time": 24.42,
          "duration": 0.92
        },
        {
          "word": "what",
          "start_time": 25.4,
          "duration": 0.16
        },
        {
          "word": "more",
          "start_time": 25.62,
          "duration": 0.14
        },
        {
          "word": "complicated",
          "start_time": 25.82,
          "duration": 0.52
        },
        {
          "word": "than",
          "start_time": 26.38,
          "duration": 0.08
        },
        {
          "word": "yours",
          "start_time": 26.5,
          "duration": 0.18
        },
        {
          "word": "is",
          "start_time": 26.74,
          "duration": 0.1
        },
        {
          "word": "predicated",
          "start_time": 26.94,
          "duration": 0.38
        },
        {
          "word": "will",
          "start_time": 27.42,
          "duration": 0.14
        },
        {
          "word": "unsecured",
          "start_time": 27.6,
          "duration": 0.5
        },
        {
          "word": "and",
          "start_time": 28.18,
          "duration": 0.1
        },
        {
          "word": "lies",
          "start_time": 28.36,
          "duration": 0.2
        },
        {
          "word": "it",
          "start_time": 28.62,
          "duration": 0.06
        },
        {
          "word": "wasn't",
          "start_time": 28.7,
          "duration": 0.24
        },
        {
          "word": "a",
          "start_time": 28.96,
          "duration": 0.04
        },
        {
          "word": "whole",
          "start_time": 29.04,
          "duration": 0.14
        },
        {
          "word": "lot",
          "start_time": 29.22,
          "duration": 0.12
        },
        {
          "word": "of",
          "start_time": 29.38,
          "duration": 0.94
        },
        {
          "word": "trust",
          "start_time": 30.42,
          "duration": 0.3
        },
        {
          "word": "there",
          "start_time": 30.78,
          "duration": 0.24
        },
        {
          "word": "you",
          "start_time": 31.08,
          "duration": 0.1
        },
        {
          "word": "know",
          "start_time": 31.22,
          "duration": 0.76
        },
        {
          "word": "heroes",
          "start_time": 32.06,
          "duration": 1.44
        },
        {
          "word": "here",
          "start_time": 33.56,
          "duration": 0.22
        },
        {
          "word": "comes",
          "start_time": 33.84,
          "duration": 0.34
        },
        {
          "word": "john",
          "start_time": 34.3,
          "duration": 0.24
        },
        {
          "word": "wayne",
          "start_time": 34.66,
          "duration": 2.38
        },
        {
          "word": "i'm",
          "start_time": 37.16,
          "duration": 0.14
        },
        {
          "word": "not",
          "start_time": 37.34,
          "duration": 0.2
        },
        {
          "word": "going",
          "start_time": 37.58,
          "duration": 0.12
        },
        {
          "word": "to",
          "start_time": 37.72,
          "duration": 0.18
        },
        {
          "word": "cry",
          "start_time": 38.06,
          "duration": 0.34
        },
        {
          "word": "about",
          "start_time": 38.5,
          "duration": 0.34
        },
        {
          "word": "a",
          "start_time": 39.5,
          "duration": 0.68
        },
        {
          "word": "i'm",
          "start_time": 40.28,
          "duration": 0.12
        },
        {
          "word": "going",
          "start_time": 40.42,
          "duration": 0.12
        },
        {
          "word": "to",
          "start_time": 40.56,
          "duration": 0.14
        },
        {
          "word": "build",
          "start_time": 40.76,
          "duration": 0.22
        },
        {
          "word": "airport",
          "start_time": 41.18,
          "duration": 0.74
        },
        {
          "word": "butcherings",
          "start_time": 41.96,
          "duration": 6.42
        },
        {
          "word": "you",
          "start_time": 48.42,
          "duration": 0.1
        },
        {
          "word": "can",
          "start_time": 48.56,
          "duration": 0.1
        },
        {
          "word": "keep",
          "start_time": 48.7,
          "duration": 0.14
        },
        {
          "word": "em",
          "start_time": 48.94,
          "duration": 0.1
        },
        {
          "word": "bottled",
          "start_time": 49.06,
          "duration": 0.3
        },
        {
          "word": "up",
          "start_time": 49.44,
          "duration": 0.12
        },
        {
          "word": "but",
          "start_time": 49.6,
          "duration": 0.1
        },
        {
          "word": "they",
          "start_time": 49.74,
          "duration": 0.2
        },
        {
          "word": "will",
          "start_time": 50.02,
          "duration": 0.24
        },
        {
          "word": "come",
          "start_time": 50.32,
          "duration": 0.12
        },
        {
          "word": "out",
          "start_time": 50.52,
          "duration": 0.16
        },
        {
          "word": "michael",
          "start_time": 50.72,
          "duration": 0.3
        },
        {
          "word": "sometimes",
          "start_time": 51.08,
          "duration": 0.64
        },
        {
          "word": "in",
          "start_time": 51.78,
          "duration": 0.06
        },
        {
          "word": "the",
          "start_time": 51.88,
          "duration": 0.1
        },
        {
          "word": "most",
          "start_time": 52.02,
          "duration": 0.36
        },
        {
          "word": "conceited",
          "start_time": 52.54,
          "duration": 4.42
        }
      ]
    },
    {
      "confidence": -977.8711547851562,
      "words": [
        {
          "word": "that",
          "start_time": 0.48,
          "duration": 0.08
        },
        {
          "word": "was",
          "start_time": 0.6,
          "duration": 0.1
        },
        {
          "word": "actually",
          "start_time": 0.78,
          "duration": 0.28
        },
        {
          "word": "example",
          "start_time": 1.12,
          "duration": 0.4
        },
        {
          "word": "of",
          "start_time": 1.58,
          "duration": 0.08
        },
        {
          "word": "how",
          "start_time": 1.76,
          "duration": 0.16
        },
        {
          "word": "not",
          "start_time": 2.0,
          "duration": 0.26
        },
        {
          "word": "so",
          "start_time": 2.3,
          "duration": 0.14
        },
        {
          "word": "amazing",
          "start_time": 2.52,
          "duration": 0.34
        },
        {
          "word": "he",
          "start_time": 2.98,
          "duration": 0.12
        },
        {
          "word": "was",
          "start_time": 3.16,
          "duration": 0.18
        },
        {
          "word": "but",
          "start_time": 3.42,
          "duration": 0.12
        },
        {
          "word": "you",
          "start_time": 3.56,
          "duration": 0.12
        },
        {
          "word": "really",
          "start_time": 3.72,
          "duration": 0.18
        },
        {
          "word": "going",
          "start_time": 3.94,
          "duration": 0.22
        },
        {
          "word": "through",
          "start_time": 4.2,
          "duration": 0.24
        },
        {
          "word": "something",
          "start_time": 4.54,
          "duration": 0.32
        },
        {
          "word": "here",
          "start_time": 4.94,
          "duration": 1.88
        },
        {
          "word": "now",
          "start_time": 6.92,
          "duration": 1.1
        },
        {
          "word": "i",
          "start_time": 8.28,
          "duration": 0.18
        },
        {
          "word": "find",
          "start_time": 8.54,
          "duration": 0.54
        },
        {
          "word": "all",
          "start_time": 9.22,
          "duration": 0.1
        },
        {
          "word": "those",
          "start_time": 9.38,
          "duration": 0.2
        },
        {
          "word": "years",
          "start_time": 9.68,
          "duration": 0.26
        },
        {
          "word": "when",
          "start_time": 10.0,
          "duration": 0.1
        },
        {
          "word": "i",
          "start_time": 10.14,
          "duration": 0.06
        },
        {
          "word": "pretended",
          "start_time": 10.26,
          "duration": 0.34
        },
        {
          "word": "to",
          "start_time": 10.64,
          "duration": 0.1
        },
        {
          "word": "cry",
          "start_time": 10.8,
          "duration": 0.2
        },
        {
          "word": "i",
          "start_time": 11.04,
          "duration": 0.02
        },
        {
          "word": "used",
          "start_time": 11.14,
          "duration": 0.12
        },
        {
          "word": "to",
          "start_time": 11.28,
          "duration": 0.08
        },
        {
          "word": "use",
          "start_time": 11.44,
          "duration": 0.14
        },
        {
          "word": "dad's",
          "start_time": 11.64,
          "duration": 0.28
        },
        {
          "word": "death",
          "start_time": 12.0,
          "duration": 0.2
        },
        {
          "word": "to",
          "start_time": 12.24,
          "duration": 0.08
        },
        {
          "word": "get",
          "start_time": 12.36,
          "duration": 0.12
        },
        {
          "word": "me",
          "start_time": 12.52,
          "duration": 0.12
        },
        {
          "word": "going",
          "start_time": 12.72,
          "duration": 0.34
        },
        {
          "word": "i",
          "start_time": 13.12,
          "duration": 0.06
        },
        {
          "word": "try",
          "start_time": 13.22,
          "duration": 0.1
        },
        {
          "word": "with",
          "start_time": 13.44,
          "duration": 0.12
        },
        {
          "word": "mom",
          "start_time": 13.62,
          "duration": 0.3
        },
        {
          "word": "but",
          "start_time": 13.96,
          "duration": 0.16
        },
        {
          "word": "i",
          "start_time": 14.18,
          "duration": 0.08
        },
        {
          "word": "just",
          "start_time": 14.28,
          "duration": 0.12
        },
        {
          "word": "end",
          "start_time": 14.52,
          "duration": 0.08
        },
        {
          "word": "up",
          "start_time": 14.68,
          "duration": 0.2
        },
        {
          "word": "smiling",
          "start_time": 14.96,
          "duration": 0.38
        },
        {
          "word": "and",
          "start_time": 15.42,
          "duration": 0.06
        },
        {
          "word": "ruining",
          "start_time": 15.58,
          "duration": 0.32
        },
        {
          "word": "it",
          "start_time": 15.96,
          "duration": 0.16
        },
        {
          "word": "but",
          "start_time": 16.18,
          "duration": 1.02
        },
        {
          "word": "it",
          "start_time": 17.38,
          "duration": 0.08
        },
        {
          "word": "feels",
          "start_time": 17.52,
          "duration": 0.24
        },
        {
          "word": "like",
          "start_time": 17.84,
          "duration": 0.26
        },
        {
          "word": "real",
          "start_time": 18.2,
          "duration": 0.3
        },
        {
          "word": "you",
          "start_time": 18.62,
          "duration": 0.12
        },
        {
          "word": "know",
          "start_time": 18.78,
          "duration": 0.28
        },
        {
          "word": "and",
          "start_time": 19.22,
          "duration": 0.22
        },
        {
          "word": "you",
          "start_time": 19.48,
          "duration": 0.12
        },
        {
          "word": "haven't",
          "start_time": 19.64,
          "duration": 0.26
        },
        {
          "word": "really",
          "start_time": 19.96,
          "duration": 0.24
        },
        {
          "word": "allowed",
          "start_time": 20.3,
          "duration": 0.24
        },
        {
          "word": "herself",
          "start_time": 20.58,
          "duration": 0.38
        },
        {
          "word": "to",
          "start_time": 21.02,
          "duration": 0.14
        },
        {
          "word": "grieve",
          "start_time": 21.22,
          "duration": 0.32
        },
        {
          "word": "much",
          "start_time": 21.64,
          "duration": 0.14
        },
        {
          "word": "at",
          "start_time": 21.84,
          "duration": 0.08
        },
        {
          "word": "all",
          "start_time": 22.0,
          "duration": 0.84
        },
        {
          "word": "moralists",
          "start_time": 22.9,
          "duration": 1.2
        },
        {
          "word": "much",
          "start_time": 24.14,
          "duration": 0.22
        },
        {
          "word": "more",
          "start_time": 24.42,
          "duration": 0.92
        },
        {
          "word": "what",
          "start_time": 25.4,
          "duration": 0.16
        },
        {
          "word": "more",
          "start_time": 25.62,
          "duration": 0.14
        },
        {
          "word": "complicated",
          "start_time": 25.82,
          "duration": 0.52
        },
        {
          "word": "than",
          "start_time": 26.38,
          "duration": 0.08
        },
        {
          "word": "yours",
          "start_time": 26.5,
          "duration": 0.18
        },
        {
          "word": "is",
          "start_time": 26.74,
          "duration": 0.1
        },
        {
          "word": "predicated",
          "start_time": 26.94,
          "duration": 0.38
        },
        {
          "word": "will",
          "start_time": 27.42,
          "duration": 0.14
        },
        {
          "word": "unsecured",
          "start_time": 27.6,
          "duration": 0.5
        },
        {
          "word": "and",
          "start_time": 28.18,
          "duration": 0.1
        },
        {
          "word": "lies",
          "start_time": 28.36,
          "duration": 0.2
        },
        {
          "word": "it",
          "start_time": 28.62,
          "duration": 0.06
        },
        {
          "word": "wasn't",
          "start_time": 28.7,
          "duration": 0.24
        },
        {
          "word": "a",
          "start_time": 28.96,
          "duration": 0.04
        },
        {
          "word": "whole",
          "start_time": 29.04,
          "duration": 0.14
        },
        {
          "word": "lot",
          "start_time": 29.22,
          "duration": 0.12
        },
        {
          "word": "of",
          "start_time": 29.38,
          "duration": 0.94
        },
        {
          "word": "trust",
          "start_time": 30.42,
          "duration": 0.3
        },
        {
          "word": "there",
          "start_time": 30.78,
          "duration": 0.24
        },
        {
          "word": "you",
          "start_time": 31.08,
          "duration": 0.1
        },
        {
          "word": "know",
          "start_time": 31.22,
          "duration": 0.76
        },
        {
          "word": "heroes",
          "start_time": 32.06,
          "duration": 1.44
        },
        {
          "word": "here",
          "start_time": 33.56,
          "duration": 0.22
        },
        {
          "word": "comes",
          "start_time": 33.84,
          "duration": 0.34
        },
        {
          "word": "john",
          "start_time": 34.3,
          "duration": 0.24
        },
        {
          "word": "wayne",
          "start_time": 34.66,
          "duration": 2.38
        },
        {
          "word": "i'm",
          "start_time": 37.16,
          "duration": 0.14
        },
        {
          "word": "not",
          "start_time": 37.34,
          "duration": 0.2
        },
        {
          "word": "going",
          "start_time": 37.58,
          "duration": 0.12
        },
        {
          "word": "to",
          "start_time": 37.72,
          "duration": 0.18
        },
        {
          "word": "cry",
          "start_time": 38.06,
          "duration": 0.34
        },
        {
          "word": "about",
          "start_time": 38.5,
          "duration": 0.34
        },
        {
          "word": "a",
          "start_time": 39.5,
          "duration": 0.68
        },
        {
          "word": "i'm",
          "start_time": 40.28,
          "duration": 0.12
        },
        {
          "word": "going",
          "start_time": 40.42,
          "duration": 0.12
        },
        {
          "word": "to",
          "start_time": 40.56,
          "duration": 0.14
        },
        {
          "word": "build",
          "start_time": 40.76,
          "duration": 0.22
        },
        {
          "word": "airport",
          "start_time": 41.18,
          "duration": 0.74
        },
        {
          "word": "butcherings",
          "start_time": 41.96,
          "duration": 6.42
        },
        {
          "word": "you",
          "start_time": 48.42,
          "duration": 0.1
        },
        {
          "word": "can",
          "start_time": 48.56,
          "duration": 0.1
        },
        {
          "word": "keep",
          "start_time": 48.7,
          "duration": 0.14
        },
        {
          "word": "him",
          "start_time": 48.9,
          "duration": 0.14
        },
        {
          "word": "bottled",
          "start_time": 49.06,
          "duration": 0.3
        },
        {
          "word": "up",
          "start_time": 49.44,
          "duration": 0.12
        },
        {
          "word": "but",
          "start_time": 49.6,
          "duration": 0.1
        },
        {
          "word": "they",
          "start_time": 49.74,
          "duration": 0.2
        },
        {
          "word": "will",
          "start_time": 50.02,
          "duration": 0.24
        },
        {
          "word": "come",
          "start_time": 50.32,
          "duration": 0.12
        },
        {
          "word": "out",
          "start_time": 50.52,
          "duration": 0.16
        },
        {
          "word": "michael",
          "start_time": 50.72,
          "duration": 0.3
        },
        {
          "word": "sometimes",
          "start_time": 51.08,
          "duration": 0.64
        },
        {
          "word": "in",
          "start_time": 51.78,
          "duration": 0.06
        },
        {
          "word": "the",
          "start_time": 51.88,
          "duration": 0.5
        },
        {
          "word": "concert",
          "start_time": 52.54,
          "duration": 4.42
        }
      ]
    }
  ]
}



let words = transcript['transcripts'][0]['words']
 
function setUp(wordList){
  for (i=0; i<wordList.length; i++){
    wordList[i]["deleted"] = false;
  }
  return wordList
}
 
words = setUp(words)
let deleteStack = []
 
// called once per deleted word
function deleteText(index, wordList){
  wordList[index]['deleted'] = true
  changeTimesDelete(index, wordList)
}
 
function changeTimesDelete(index, wordList){
  // save index and initial time diff to stack
  let initial_diff = wordList[index+1]["start_time"] - wordList[index]["start_time"]
  let delOperation = [index, initial_diff]
  deleteStack.push(delOperation)

  prev = wordList[index+1]['start_time']
  wordList[index+1]['start_time'] = wordList[index]['start_time']
 
  // start time (current) = start time(prev) + diff (current, prev)
  for (i=index+2; i< wordList.length; i++){
    time_diff = wordList[i]['start_time'] - prev
    prev = wordList[i]['start_time']
    wordList[i]['start_time'] = time_diff + wordList[i-1]['start_time']
  }
 
}

// undoes ONE delete operation
function undoDelete(operationStack, wordList){
  // no deletes
  if (operationStack.length == 0){
    return
  }
  let delOperation = operationStack.pop()
  let index = delOperation[0]
  let diff = delOperation[1]
  let next_diff
  
  for (i=index; i<wordList.length-1; i++){
    if (i < wordList.length-2){
      next_diff = wordList[i+2]['start_time'] - wordList[i+1]['start_time']
    }
    wordList[i+1]['start_time'] = wordList[i]['start_time'] + diff
    diff = next_diff
  }

  return wordList

}


start_times = words.map(({start_time}) => start_time);
console.log(start_times.slice(120, words.length-1));
deleteText(121, words)
start_times = words.map(({start_time}) => start_time);
console.log(start_times.slice(120, words.length-1));



undoDelete(deleteStack, words)
start_times = words.map(({start_time}) => start_time);
console.log(start_times.slice(120, words.length-1))

